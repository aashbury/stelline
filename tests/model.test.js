...
test("widgets: defaults from the old card settings, a tile's own on top, old clock knobs still count", () => {
  ...
  // the full-text level is gone; a tile that chose it now shows titles
  assert.equal(d.notifications.detail, "summaries")
  assert.equal(M.widgetsOf({}, M.defaults()).notifications.detail, "counts")
...
test("agent sessions: Claude's own status wins, waitingFor tells needs from waiting, others go by activity", () => {
  ...
  assert.equal(M.widgetsOf({}, {}).agent.detail, "titles")
  assert.equal(M.widgetsOf({ widgets: { agent: { detail: "state" } } }, {}).agent.detail, "state")
