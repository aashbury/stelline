...
| panel | **Shuffle** in the gallery's header | the tiles become checkboxes; the shuffle plays the ticked ones (none ticked: all of them). Under the header, **Select all** ticks every saver (then **Clear** unticks them), and **Next saver** sets how long each one plays before the next while the screen is up — *each time it starts* (the default: one saver per session), or every 1 to 30 minutes. A saver a rule has chosen stays put. |
...
An effect may draw outside the word. The weather of `storm`, `shockwave`,
`beams` and `cascade` has a grid of its own — the whole screen, at the dot
size the art would have at Full, drawn behind it — so `beams` sweep across
empty space and `cascade` rains past the letters rather than only on them,
whatever size the art is; the others carry a margin round the word, in its
own frame of reference. Each brightness is emitted at its own left edge and shifted back
...
`effects`, `order` (`sequence`/`shuffle`), `size` (`s`/`m`/`l`/`full` — shown as XL — or `fill`; the older `fit: "cover"` reads as `fill`), `motion`
